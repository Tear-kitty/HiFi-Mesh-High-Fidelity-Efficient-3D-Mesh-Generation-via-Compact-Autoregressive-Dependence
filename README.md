...
The recommended dependency policy is:

```text
requirements.txt              # common Python dependencies
requirements.lock.txt         # exact environment snapshot, optional
```

Do not blindly copy every package from `pip list` into `requirements.txt`. Many packages in a working environment are transitive dependencies, CUDA wheel internals, or unrelated tools. In particular, keep the following separate from `requirements.txt`:

```text
torch / torchvision / torchaudio  # install from the correct PyTorch CUDA index
flash-attn                        # install after torch with --no-build-isolation
meto                              # install from the local ./meto source tree
```

To export an exact lock file from a working environment:
...
A good open-source practice is to maintain a clean `requirements.txt` manually and provide `requirements.lock.txt` only as a known-working reference.

## Troubleshooting

### `ImportError: opt.use_meto=True requires EdgeRunner's local ./meto tokenizer`

Make sure the repository contains the real EdgeRunner tokenizer source:

```text
./meto/setup.py
./meto/meto/__init__.py
./meto/src/bindings.cpp
```

Then reinstall it:

```bash
python -m pip uninstall -y meto
bash scripts/install_meto.sh
```

If Python still imports the wrong package, launch with:
...
Install PyTorch first, then install `flash-attn` with:

```bash
python -m pip install flash-attn==2.8.3 --no-build-isolation
```

If it still fails, check your CUDA toolkit, PyTorch CUDA version, compiler version, and GPU architecture. Training is expected to run best on Ampere or newer NVIDIA GPUs.

### `libGL.so` or OpenCV import error

Install the common OpenGL/Mesa runtime libraries:

```bash
sudo apt-get update
sudo apt-get install -y libgl1-mesa-dev libglib2.0-0 libsm6 libxext6 libxrender1
```
